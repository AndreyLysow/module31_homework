export class LocalStorageManager {
 ...
 removeTasksFromStorage(userId, tasksToRemove) {
  const tasksInStorage = this.getTasksFromStorage(userId);

  for (const listName in tasksInStorage) {
    if (tasksInStorage.hasOwnProperty(listName)) {
      tasksInStorage[listName] = tasksInStorage[listName].filter((task) => !tasksToRemove.includes(task.id));
    }
  }

  localStorage.setItem(userId, JSON.stringify(tasksInStorage));
}
